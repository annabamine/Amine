...
# --- CONFIGURATION NAVIGATION BARRE LATÉRALE ---
# On gère le layout de manière dynamique (wide pour le comparateur, centered pour l'analyse)
if "current_page" not in st.session_state:
    st.session_state.current_page = "Analyse Individuelle"

# Barre de navigation latérale de Streamlit
menu = st.sidebar.radio(
    "Navigation",
    ["Analyse Individuelle", "⚖️ Comparateur d'Entreprises"]
)

# 1. Toujours en premier (ajustement du layout selon le menu)
if menu == "⚖️ Comparateur d'Entreprises":
    st.set_page_config(page_title="Value Quest", layout="wide")
else:
    st.set_page_config(page_title="Value Quest", layout="centered")


# 2. Barre de titre globale (Logique Logo + HTML)
...
# --- FONCTIONS CACHÉES ET LOGIQUE DE RECHERCHE ---
...
# ==============================================================================
# PAGE 1 : ANALYSE INDIVIDUELLE (TON CODE EXISTANT)
# ==============================================================================
if menu == "Analyse Individuelle":
    ...
    if ticker:
        try:
            ...
            with tab4:
                ...
                try:
                    sidebar_val = infos.get('heldPercentInsiders')
                    if sidebar_val is not None and sidebar_val != 0:
                        if sidebar_val < 1:
                            insider_pct = sidebar_val * 100
                        elif sidebar_val > 100:
                            insider_pct = sidebar_val / 100
                        else:
                            insider_pct = sidebar_val
...
# ==============================================================================
# PAGE 2 : COMPARATEUR MULTI-ENTREPRISES (NOUVEAU BLOC INTÉGRÉ)
# ==============================================================================
elif menu == "⚖️ Comparateur d'Entreprises":
    st.title("⚖️ Comparateur d'Entreprises")
    st.write("Entrez et sélectionnez jusqu'à 3 tickers pour juxtaposer leurs ratios financiers essentiels.")

    # Composant multiselect interactif (L'utilisateur peut ajouter ses propres symboles)
    entreprises_choisies = st.multiselect(
        "Sélectionnez ou tapez les tickers à analyser (max 3) :",
        options=["AAPL", "MSFT", "GOOGL", "META", "NVDA", "TTE", "SAN.PA", "BNP.PA", "PYPL"],
        default=["AAPL", "MSFT"],
        max_selections=3
    )

    if len(entreprises_choisies) < 2:
        st.info("💡 Veuillez sélectionner au moins 2 entreprises pour lancer la comparaison des données.")
    else:
        donnees_comparatives = {}

        # Spinner de chargement pendant la récupération des API Yahoo
        with st.spinner("Récupération des métriques financières en cours..."):
            for t_name in entreprises_choisies:
                try:
                    ...
                    # Construction du dictionnaire de données sectorielles et fondamentales
                    donnees_comparatives[t_name] = {
                        "Nom de l'entreprise": info_comp.get("longName", "N/A"),
                        "Secteur d'activité": info_comp.get("sector", "N/A"),
                        "Prix Actuel": f"{info_comp.get('currentPrice', 0):.2f} {devise_comp}" if info_comp.get('currentPrice') else "N/A",
                        "Capitalisation (Mds)": f"{info_comp.get('marketCap', 0) / 1e9:.2f} Mds {devise_comp}" if info_comp.get('marketCap') else "N/A",
                        ...
                        "Price to Book (P/B)": round(info_comp.get("priceToBook"), 2) if isinstance(info_comp.get("priceToBook"), (int, float)) else "N/A",
                        ...
                        "Rendement Dividende (%)": f"{info_comp.get('dividendYield', 0) * 100:.2f}%" if info_comp.get('dividendYield') else "0.00%",
                        "Debt to Equity": f"{info_comp.get('debtToEquity', 0):.2f}%" if info_comp.get('debtToEquity') else "N/A",
                    }
                except Exception as e:
                    st.error(f"Erreur de chargement pour le ticker {t_name} : {e}")

        if donnees_comparatives:
            # Transformation en DataFrame Pandas pour mise en tableau
            df_comparatif = pd.DataFrame(donnees_comparatives)
            
            st.subheader("📊 Matrice de Comparaison des Ratios")
            st.dataframe(df_comparatif, use_container_width=True)
            
            # --- BLOC DE VISUALISATION GRAPHIQUE ---
            st.markdown("---")
            st.subheader("📈 Comparatif Visuel : PER vs Forward PER")
            
            # Extraction propre des PER pour le graphique (uniquement s'ils sont numériques)
            graph_data = []
            for t_name, metrics in donnees_comparatives.items():
                t_per = metrics["PER (Trailing)"]
                t_fper = metrics["Forward PER"]
                
                # On remplace par 0 si le ratio est indisponible pour ne pas faire planter matplotlib
                val_per = t_per if isinstance(t_per, (int, float)) else 0
                val_fper = t_fper if isinstance(t_fper, (int, float)) else 0
                
                graph_data.append({
                    "Entreprise": t_name,
                    "PER (Trailing)": val_per,
                    "Forward PER": val_fper
                })
            
            df_graph = pd.DataFrame(graph_data).set_index("Entreprise")
            
            # Affichage d'un graphique à barres groupées directement avec Streamlit
